  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, background: C.bg, borderRadius: 12, padding: "4px 6px" }}>
        <button onClick={() => onChange(Math.max(min, value - 1))} style={{ width: 24, height: 24, borderRadius: 8, border: "none", background: C.card, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <Minus size={12} color={C.label} />
        </button>
        <span style={{ fontSize: 13, fontWeight: 700, color: C.label, minWidth: 16, textAlign: "center" }}>{value}</span>
        <button onClick={() => onChange(Math.min(max, value + 1))} style={{ width: 24, height: 24, borderRadius: 8, border: "none", background: C.card, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
...
function AddonChoicePicker({ choices, value, onChange, accent }: {
  choices: ServiceChoiceItem[]; value: string | undefined; onChange: (id: string) => void; accent: string;
}) {
  return (
        ...
        {choices.map(c => {
          ...
          return (
            <button key={c.id} onClick={() => onChange(c.id)}
              style={{ padding: "6px 12px", borderRadius: 16, border: selected ? "none" : `1px solid ${C.sep}`, background: selected ? accent : C.card, color: selected ? "#fff" : C.label, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
          ...
          <Card key={s.id}>
            ...
            <div style={{ padding: "14px 16px" }}>
              ...
              {s.priceType === "choice" && s.choices && (
                <AddonChoicePicker choices={s.choices} value={getChoiceId(s)} onChange={(id) => setChoiceId(s, id)} accent={accent} />
              )}
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, marginTop: 14 }}>
                {(s.priceType === "per_day" || s.priceType === "per_unit") && (
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: C.label, flex: 1 }}>{tr("addons_total")}: {fmtAddonPrice(totalAmount)}</p>
                )}
                <button
